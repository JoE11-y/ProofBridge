...
"[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-lib/identifiers.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
...
"[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-lib/style.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
...
"[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-conv-api/transformers.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
...
exports.utf8 = {
    encode (chars) {
        ...
        chars.forEach((char)=>{
            if (char >= 0 && char <= 0x7f) {
                ...
                throw new RangeError("utf8.encode: character out of range: char: ".concat(char));
            }
        });
    ...
    decode (buf, bom) {
        ...
        while(i < len){
            ...
            if (c > 0x10ffff) {
                const at = "byte[".concat(i, "]");
                if (c === ILL_FORMED) {
                    throw new RangeError("utf8.decode: ill-formed UTF8 byte sequence found at: ".concat(at));
                }
                if (c === TRAILING) {
                    throw new RangeError("utf8.decode: illegal trailing byte found at: ".concat(at));
                }
                if (c === RANGE) {
                    throw new RangeError("utf8.decode: code point out of range found at: ".concat(at));
                }
                if (c === NON_SHORTEST) {
                    throw new RangeError("utf8.decode: non-shortest form found at: ".concat(at));
                }
                throw new RangeError("utf8.decode: unrecognized error found at: ".concat(at));
            }
            chars.push(c);
            i += inc;
        }
...
exports.utf16be = {
    encode (chars) {
        ...
        for(let i = 0; i < chars.length; i += 1){
            ...
            if (char >= 0 && char <= 0xd7ff || char >= 0xe000 && char <= 0xffff) {
                ...
                throw new RangeError("utf16be.encode: UTF16BE value out of range: char[".concat(i, "]: ").concat(char));
            }
        }
        return Buffer.from(bytes);
    },
    decode (buf, bom) {
        /* assumes caller has insured that buf is a Buffer of bytes */ if (buf.length % 2 > 0) {
            throw new RangeError("utf16be.decode: data length must be even multiple of 2: length: ".concat(buf.length));
        }
        const chars = [];
        const len = buf.length;
        let i = bom ? 2 : 0;
        ...
        while(i < len){
            ...
            while(TRUE){
                ...
                /* if we fall through to here, it is an ill-formed sequence */ throw new RangeError("utf16be.decode: ill-formed UTF16BE byte sequence found: byte[".concat(i, "]"));
            }
            chars[j++] = c;
            i += inc;
        }
...
exports.utf16le = {
    encode (chars) {
        ...
        for(let i = 0; i < chars.length; i += 1){
            ...
            if (char >= 0 && char <= 0xd7ff || char >= 0xe000 && char <= 0xffff) {
                ...
                throw new RangeError("utf16le.encode: UTF16LE value out of range: char[".concat(i, "]: ").concat(char));
            }
        }
        return Buffer.from(bytes);
    },
    decode (buf, bom) {
        /* assumes caller has insured that buf is a Buffer of bytes */ if (buf.length % 2 > 0) {
            throw new RangeError("utf16le.decode: data length must be even multiple of 2: length: ".concat(buf.length));
        }
        const chars = [];
        const len = buf.length;
        let i = bom ? 2 : 0;
        ...
        while(i < len){
            ...
            while(TRUE){
                ...
                /* if we fall through to here, it is an ill-formed sequence */ throw new RangeError("utf16le.decode: ill-formed UTF16LE byte sequence found: byte[".concat(i, "]"));
            }
            chars[j++] = c;
            i += inc;
        }
...
exports.utf32be = {
    encode (chars) {
        ...
        chars.forEach((char)=>{
            if (char >= 0xd800 && char <= 0xdfff || char > 0x10ffff) {
                throw new RangeError("utf32be.encode: UTF32BE character code out of range: char[".concat(i / 4, "]: ").concat(char));
            }
    ...
    decode (buf, bom) {
        /* caller to insure buf is a Buffer of bytes */ if (buf.length % 4 > 0) {
            throw new RangeError("utf32be.decode: UTF32BE byte length must be even multiple of 4: length: ".concat(buf.length));
        }
        const chars = [];
        let i = bom ? 4 : 0;
        for(; i < buf.length; i += 4){
            ...
            if (char >= 0xd800 && char <= 0xdfff || char > 0x10ffff) {
                throw new RangeError("utf32be.decode: UTF32BE character code out of range: char[".concat(i / 4, "]: ").concat(char));
            }
            chars.push(char);
        }
        return chars;
...
exports.utf32le = {
    encode (chars) {
        ...
        chars.forEach((char)=>{
            if (char >= 0xd800 && char <= 0xdfff || char > 0x10ffff) {
                throw new RangeError("utf32le.encode: UTF32LE character code out of range: char[".concat(i / 4, "]: ").concat(char));
            }
            buf[i++] = char & mask[8];
            buf[i++] = char >> 8 & mask[8];
            buf[i++] = char >> 16 & mask[8];
    ...
    decode (buf, bom) {
        /* caller to insure buf is a Buffer of bytes */ if (buf.length % 4 > 0) {
            throw new RangeError("utf32be.decode: UTF32LE byte length must be even multiple of 4: length: ".concat(buf.length));
        }
        const chars = [];
        let i = bom ? 4 : 0;
        for(; i < buf.length; i += 4){
...
exports.uint7 = {
    encode (chars) {
        ...
        for(let i = 0; i < chars.length; i += 1){
            if (chars[i] > 0x7f) {
                throw new RangeError("uint7.encode: UINT7 character code out of range: char[".concat(i, "]: ").concat(chars[i]));
            }
            buf[i] = chars[i];
        }
        return buf;
    ...
    decode (buf) {
        ...
        for(let i = 0; i < buf.length; i += 1){
            if (buf[i] > 0x7f) {
                throw new RangeError("uint7.decode: UINT7 character code out of range: byte[".concat(i, "]: ").concat(buf[i]));
            }
            chars[i] = buf[i];
        }
        return chars;
...
exports.uint8 = {
    encode (chars) {
        ...
        for(let i = 0; i < chars.length; i += 1){
            if (chars[i] > 0xff) {
                throw new RangeError("uint8.encode: UINT8 character code out of range: char[".concat(i, "]: ").concat(chars[i]));
            }
            buf[i] = chars[i];
        }
        return buf;
...
exports.uint16be = {
    encode (chars) {
        ...
        chars.forEach((char)=>{
            if (char > 0xffff) {
                throw new RangeError("uint16be.encode: UINT16BE character code out of range: char[".concat(i / 2, "]: ").concat(char));
            }
            buf[i++] = char >> 8 & mask[8];
            buf[i++] = char & mask[8];
        });
    ...
    decode (buf) {
        if (buf.length % 2 > 0) {
            throw new RangeError("uint16be.decode: UINT16BE byte length must be even multiple of 2: length: ".concat(buf.length));
        }
        const chars = [];
...
exports.uint16le = {
    encode (chars) {
        ...
        chars.forEach((char)=>{
            if (char > 0xffff) {
                throw new RangeError("uint16le.encode: UINT16LE character code out of range: char[".concat(i / 2, "]: ").concat(char));
            }
            buf[i++] = char & mask[8];
            buf[i++] = char >> 8 & mask[8];
        });
    ...
    decode (buf) {
        if (buf.length % 2 > 0) {
            throw new RangeError("uint16le.decode: UINT16LE byte length must be even multiple of 2: length: ".concat(buf.length));
        }
        const chars = [];
        for(let i = 0; i < buf.length; i += 2){
...
exports.uint32be = {
    ...
    decode (buf) {
        if (buf.length % 4 > 0) {
            throw new RangeError("uint32be.decode: UINT32BE byte length must be even multiple of 4: length: ".concat(buf.length));
        }
        const chars = [];
...
exports.uint32le = {
    ...
    decode (buf) {
        /* caller to insure buf is a Buffer of bytes */ if (buf.length % 4 > 0) {
            throw new RangeError("uint32le.decode: UINT32LE byte length must be even multiple of 4: length: ".concat(buf.length));
        }
        const chars = [];
        for(let i = 0; i < buf.length; i += 4){
...
exports.escaped = {
    ...
    encode (chars) {
        ...
        for(let i = 0; i < chars.length; i += 1){
            ...
            if (char === 96) {
                ...
                let str = '';
                if (char >= 0 && char <= 31) {
                    str += "`x".concat(ascii[char]);
                } else if (char >= 127 && char <= 255) {
                    str += "`x".concat(ascii[char]);
                } else if (char >= 0x100 && char <= 0xffff) {
                    str += "`u".concat(ascii[char >> 8 & mask[8]]).concat(ascii[char & mask[8]]);
                    ...
                    str += "".concat(ascii[char >> 16 & mask[8]] + ascii[char >> 8 & mask[8]] + ascii[char & mask[8]], "}");
                } else {
    ...
    decode (buf) {
        ...
        while(i < len){
            ...
            if (error) {
                throw new Error("escaped.decode: ill-formed escape sequence at buf[".concat(i, "]"));
            }
        }
        return chars;
    }
...
exports.base64 = {
        ...
        /* remove white space and ctrl characters, validate & translate characters */ function validate(buf) {
            ...
            for(let i = 0; i < buf.length; i += 1){
                ...
                while(TRUE){
                    ...
                    /* invalid character */ throw new RangeError("base64.decode: invalid character buf[".concat(i, "]: ").concat(char));
                }
            }
            /* validate length */ if (chars.length % 4 > 0) {
                throw new RangeError("base64.decode: string length not integral multiple of 4: ".concat(chars.length));
            }
            /* validate tail */ switch(tail){
                ...
                case 2:
                    ...
                    break;
                default:
                    throw new RangeError("base64.decode: more than two tail characters found: ".concat(tail));
            }
            return {
                tail,
    ...
    toString (buf) {
        if (buf.length % 4 > 0) {
            throw new RangeError("base64.toString: input buffer length not multiple of 4: ".concat(buf.length));
        }
        let str = '';
        let lineLen = 0;
        function buildLine(c1, c2, c3, c4) {
            switch(lineLen){
                case 76:
                    str += "\r\n".concat(c1).concat(c2).concat(c3).concat(c4);
                    lineLen = 4;
                    break;
                case 75:
                    str += "".concat(c1, "\r\n").concat(c2).concat(c3).concat(c4);
                    lineLen = 3;
                    break;
                case 74:
                    str += "".concat(c1 + c2, "\r\n").concat(c3).concat(c4);
        ...
        for(let i = 0; i < buf.length; i += 4){
            for(let j = i; j < i + 4; j += 1){
                if (!validate(buf[j])) {
                    throw new RangeError("base64.toString: buf[".concat(j, "]: ").concat(buf[j], " : not valid base64 character code"));
                }
            }
...
"[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-conv-api/converter.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
...
//
const { Buffer } = __turbopack_context__.r("[project]/node_modules/.pnpm/buffer@6.0.3/node_modules/buffer/index.js [app-client] (ecmascript)");
const trans = __turbopack_context__.r("[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-conv-api/transformers.js [app-client] (ecmascript)");
        ...
        case UTF16BE:
            ...
            if (buf.length >= 2) {
                if (buf[0] === 0xfe && buf[1] === 0xff) {
                    ...
                    throw new TypeError('src type: "'.concat(UTF16BE, '" specified but BOM is for "').concat(UTF16LE, '"'));
                }
            }
            break;
        case UTF16LE:
            ...
            if (buf.length >= 0) {
                if (buf[0] === 0xfe && buf[1] === 0xff) {
                    throw new TypeError('src type: "'.concat(UTF16LE, '" specified but BOM is for "').concat(UTF16BE, '"'));
                } else if (buf[0] === 0xff && buf[1] === 0xfe) {
                    src.bom = 2;
                }
            }
            break;
        default:
            throw new TypeError('UTF16 BOM: src type "'.concat(src.type, '" unrecognized'));
    }
};
    ...
    switch(src.type){
        ...
        case UTF32BE:
            ...
            if (buf.length >= 4) {
                ...
                if (buf[0] === 0xff && buf[1] === 0xfe && buf[2] === 0 && buf[3] === 0) {
                    throw new TypeError("src type: ".concat(UTF32BE, " specified but BOM is for ").concat(UTF32LE, '"'));
                }
            }
            break;
        case UTF32LE:
            ...
            if (buf.length >= 4) {
                if (buf[0] === 0 && buf[1] === 0 && buf[2] === 0xfe && buf[3] === 0xff) {
                    throw new TypeError('src type: "'.concat(UTF32LE, '" specified but BOM is for "').concat(UTF32BE, '"'));
                }
                if (buf[0] === 0xff && buf[1] === 0xfe && buf[2] === 0 && buf[3] === 0) {
                    src.bom = 4;
                }
            ...
            break;
        default:
            throw new TypeError('UTF32 BOM: src type "'.concat(src.type, '" unrecognized'));
    }
};
...
const validateSrc = function validateSrc(type, data) {
    ...
    if (ret.base64) {
        /* handle base 64 */ if (ret.type === STRING) {
            throw new TypeError('type: "'.concat(type, ' "BASE64:" prefix not allowed with type ').concat(STRING));
        }
        if (Buffer.isBuffer(data)) {
            ...
            throw new TypeError('type: "'.concat(type, " unrecognized data type: typeof(data): ").concat(typeof data));
        }
    } else {
        ret.data = data;
    }
    switch(ret.type){
        ...
        case ESCAPED:
            break;
        default:
            throw new TypeError('type: "'.concat(type, '" not recognized'));
    }
    if (ret.type === STRING) {
        if (typeof ret.data !== 'string') {
            throw new TypeError('type: "'.concat(type, '" but data is not a string'));
        }
    } else if (!Buffer.isBuffer(ret.data)) {
        throw new TypeError('type: "'.concat(type, '" but data is not a Buffer'));
    }
    return ret;
};
...
const validateDst = function validateDst(type, chars) {
    ...
    if (!Array.isArray(chars)) {
        throw new TypeError('dst chars: not array: "'.concat(typeof chars));
    }
    if (typeof type !== 'string') {
        throw new TypeError('dst type: not string: "'.concat(typeof type));
    }
    const ret = getType(type.toUpperCase());
    switch(ret.type){
        case UTF8:
        ...
        case STRING:
            if (ret.base64) {
                throw new TypeError('":BASE64" suffix not allowed with type '.concat(STRING));
            }
            break;
        case ASCII:
            ret.type = UINT7;
        ...
        case UINT32:
            ...
            break;
        default:
            throw new TypeError('dst type unrecognized: "'.concat(type, '" : must have form [crlf:|lf:]type[:base64]'));
    }
    return ret;
};
...
const encode = function encode(type, chars) {
    switch(type){
        ...
        default:
            throw new TypeError('encode type "'.concat(type, '" not recognized'));
    }
};
...
const decode = function decode(src) {
    switch(src.type){
        ...
        default:
            throw new TypeError('decode type "'.concat(src.type, '" not recognized'));
    }
};
...
"[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-lib/emitcss.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
...
"[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-lib/utilities.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
...
const style = __turbopack_context__.r("[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-lib/style.js [app-client] (ecmascript)");
const converter = __turbopack_context__.r("[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-conv-api/converter.js [app-client] (ecmascript)");
const emitCss = __turbopack_context__.r("[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-lib/emitcss.js [app-client] (ecmascript)");
const id = __turbopack_context__.r("[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-lib/identifiers.js [app-client] (ecmascript)");
...
exports.htmlToPage = function(html, titleArg) {
    ...
    if (typeof html !== 'string') {
        throw new Error("".concat(thisFileName, "htmlToPage: input HTML is not a string"));
    }
    ...
    page += "<title>".concat(title, "</title>\n");
    page += '<style>\n';
    page += emitCss();
    page += '</style>\n';
    page += '</head>\n<body>\n';
    page += "<p>".concat(new Date(), "</p>\n");
    page += html;
    page += '</body>\n</html>\n';
    return page;
};
...
exports.parserResultToHtml = function(result, caption) {
    ...
    if (result.success === true) {
        success = '<span class="'.concat(style.CLASS_MATCH, '">true</span>');
    } else {
        success = '<span class="'.concat(style.CLASS_NOMATCH, '">false</span>');
    }
    if (result.state === id.EMPTY) {
        state = '<span class="'.concat(style.CLASS_EMPTY, '">EMPTY</span>');
    } else if (result.state === id.MATCH) {
        state = '<span class="'.concat(style.CLASS_MATCH, '">MATCH</span>');
    } else if (result.state === id.NOMATCH) {
        state = '<span class="'.concat(style.CLASS_NOMATCH, '">NOMATCH</span>');
    } else {
        state = '<span class="'.concat(style.CLASS_NOMATCH, '">unrecognized</span>');
    }
    let html = '';
    html += '<table class="'.concat(style.CLASS_STATE, '">\n');
    if (cap) {
        html += "<caption>".concat(cap, "</caption>\n");
    }
    html += '<tr><th>state item</th><th>value</th><th>description</th></tr>\n';
    html += "<tr><td>parser success</td><td>".concat(success, "</td>\n");
    html += '<td><span class="'.concat(style.CLASS_MATCH, '">true</span> if the parse succeeded,\n');
    html += ' <span class="'.concat(style.CLASS_NOMATCH, '">false</span> otherwise');
    html += '<br><i>NOTE: for success, entire string must be matched</i></td></tr>\n';
    html += "<tr><td>parser state</td><td>".concat(state, "</td>\n");
    html += '<td><span class="'.concat(style.CLASS_EMPTY, '">EMPTY</span>, ');
    html += '<span class="'.concat(style.CLASS_MATCH, '">MATCH</span> or \n');
    html += '<span class="'.concat(style.CLASS_NOMATCH, '">NOMATCH</span></td></tr>\n');
    html += "<tr><td>string length</td><td>".concat(result.length, "</td><td>length of the input (sub)string</td></tr>\n");
    html += "<tr><td>matched length</td><td>".concat(result.matched, "</td><td>number of input string characters matched</td></tr>\n");
    html += "<tr><td>max matched</td><td>".concat(result.maxMatched, "</td><td>maximum number of input string characters matched</td></tr>\n");
    html += "<tr><td>max tree depth</td><td>".concat(result.maxTreeDepth, "</td><td>maximum depth of the parse tree reached</td></tr>\n");
    html += "<tr><td>node hits</td><td>".concat(result.nodeHits, "</td><td>number of parse tree node hits (opcode function calls)</td></tr>\n");
    html += "<tr><td>input length</td><td>".concat(result.inputLength, "</td><td>length of full input string</td></tr>\n");
    html += "<tr><td>sub-string begin</td><td>".concat(result.subBegin, "</td><td>sub-string first character index</td></tr>\n");
    html += "<tr><td>sub-string end</td><td>".concat(result.subEnd, "</td><td>sub-string end-of-string index</td></tr>\n");
    html += "<tr><td>sub-string length</td><td>".concat(result.subLength, "</td><td>sub-string length</td></tr>\n");
    html += '</table>\n';
    return html;
};
...
exports.charsToDec = function(chars, beg, len) {
    let ret = '';
    if (!Array.isArray(chars)) {
        throw new Error("".concat(thisFileName, "charsToDec: input must be an array of integers"));
    }
...
exports.charsToHex = function(chars, beg, len) {
    let ret = '';
    if (!Array.isArray(chars)) {
        throw new Error("".concat(thisFileName, "charsToHex: input must be an array of integers"));
    }
    const bounds = getBounds(chars.length, beg, len);
    if (bounds.end > bounds.beg) {
        ret += "\\x".concat(exports.charToHex(chars[bounds.beg]));
        for(let i = bounds.beg + 1; i < bounds.end; i += 1){
            ret += ",\\x".concat(exports.charToHex(chars[i]));
        }
    }
    return ret;
};
exports.charsToHtmlEntities = function(chars, beg, len) {
    ...
    if (bounds.end > bounds.beg) {
        for(let i = bounds.beg; i < bounds.end; i += 1){
            ret += "&#x".concat(chars[i].toString(16), ";");
        }
    }
    return ret;
};
...
exports.charsToUnicode = function(chars, beg, len) {
    let ret = '';
    if (!Array.isArray(chars)) {
        throw new Error("".concat(thisFileName, "charsToUnicode: input must be an array of integers"));
    }
    const bounds = getBounds(chars.length, beg, len);
    if (bounds.end > bounds.beg) {
        for(let i = bounds.beg; i < bounds.end; i += 1){
            if (isUnicode(chars[i])) {
                ...
                ret += " U+".concat(exports.charToHex(chars[i]));
            }
        }
    }
    return ret;
...
exports.charsToJsUnicode = function(chars, beg, len) {
    let ret = '';
    if (!Array.isArray(chars)) {
        throw new Error("".concat(thisFileName, "charsToJsUnicode: input must be an array of integers"));
    }
    const bounds = getBounds(chars.length, beg, len);
    if (bounds.end > bounds.beg) {
        ret += "\\u".concat(exports.charToHex(chars[bounds.beg]));
        for(let i = bounds.beg + 1; i < bounds.end; i += 1){
            ret += ",\\u".concat(exports.charToHex(chars[i]));
        }
    }
...
exports.charsToAscii = function(chars, beg, len) {
    let ret = '';
    if (!Array.isArray(chars)) {
        throw new Error("".concat(thisFileName, "charsToAscii: input must be an array of integers"));
    }
    const bounds = getBounds(chars.length, beg, len);
    for(let i = bounds.beg; i < bounds.end; i += 1){
        const char = chars[i];
        if (char >= 32 && char <= 126) {
            ...
            ret += "\\x".concat(exports.charToHex(char));
        }
    }
    return ret;
};
...
exports.charsToAsciiHtml = function(chars, beg, len) {
    if (!Array.isArray(chars)) {
        throw new Error("".concat(thisFileName, "charsToAsciiHtml: input must be an array of integers"));
    }
    let html = '';
    let char;
    const bounds = getBounds(chars.length, beg, len);
    for(let i = bounds.beg; i < bounds.end; i += 1){
        char = chars[i];
        if (char < 32 || char === 127) {
            /* control characters */ html += '<span class="'.concat(style.CLASS_CTRLCHAR, '">').concat(exports.asciiChars[char], "</span>");
        } else if (char > 127) {
            /* non-ASCII */ html += '<span class="'.concat(style.CLASS_CTRLCHAR, '">U+').concat(exports.charToHex(char), "</span>");
        } else {
...
"[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-lib/ast.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
...
module.exports = function exportsAst() {
    const id = __turbopack_context__.r("[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-lib/identifiers.js [app-client] (ecmascript)");
    const utils = __turbopack_context__.r("[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-lib/utilities.js [app-client] (ecmascript)");
    ...
    this.toXml = function toSml(modeArg) {
        ...
        xml += "<!-- input string, ".concat(caption, " -->\n");
        xml += indent(depth + 2);
        xml += display(chars);
        xml += '\n';
        records.forEach((rec)=>{
...
"[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-lib/circular-buffer.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
...
module.exports = function exportsCircularBuffer() {
    ...
    this.init = function init(size) {
        if (typeof size !== 'number' || size <= 0) {
            throw new Error("".concat(thisFileName, "init: circular buffer size must an integer > 0"));
        }
...
"[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-lib/parser.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
...
module.exports = function parser() {
    ...
    const evaluateRule = function evaluateRule(ruleIndex, phraseIndex, sysData) {
        const functionName = "".concat(thisFileName, "evaluateRule(): ");
        if (ruleIndex >= rules.length) {
            throw new Error("".concat(functionName, "rule index: ").concat(ruleIndex, " out of range"));
        }
        if (phraseIndex >= charsEnd) {
            throw new Error("".concat(functionName, "phrase index: ").concat(phraseIndex, " out of range"));
        }
    ...
    const evaluateUdt = function(udtIndex, phraseIndex, sysData) {
        const functionName = "".concat(thisFileName, "evaluateUdt(): ");
        if (udtIndex >= udts.length) {
            throw new Error("".concat(functionName, "udt index: ").concat(udtIndex, " out of range"));
        }
        if (phraseIndex >= charsEnd) {
            throw new Error("".concat(functionName, "phrase index: ").concat(phraseIndex, " out of range"));
        }
    ...
    /* object for maintaining a stack of back reference frames */ const backRef = function() {
        ...
        this.pop = function pop(lengthArg) {
            ...
            if (length < 1 || length > stack.length) {
                throw new Error("".concat(thisFileName, "backRef.pop(): bad length: ").concat(length));
            }
            stack.length = length;
    ...
    /* called by parse() to initialize the AST object, if one has been defined */ const initializeAst = function() {
        const functionName = "".concat(thisFileName, "initializeAst(): ");
        const TRUE = true;
        while(TRUE){
            ...
            if (thisThis.ast.astObject !== 'astObject') {
                throw new Error("".concat(functionName, "ast object not recognized"));
            }
            break;
        }
        if (thisThis.ast !== null) {
    ...
    /* called by parse() to initialize the trace object, if one has been defined */ const initializeTrace = function() {
        const functionName = "".concat(thisFileName, "initializeTrace(): ");
        const TRUE = true;
        while(TRUE){
            ...
            if (thisThis.trace.traceObject !== 'traceObject') {
                throw new Error("".concat(functionName, "trace object not recognized"));
            }
            break;
        }
        if (thisThis.trace !== null) {
    ...
    /* called by parse() to initialize the statistics object, if one has been defined */ const initializeStats = function() {
        const functionName = "".concat(thisFileName, "initializeStats(): ");
        const TRUE = true;
        while(TRUE){
            ...
            if (thisThis.stats.statsObject !== 'statsObject') {
                throw new Error("".concat(functionName, "stats object not recognized"));
            }
            break;
        }
        if (thisThis.stats !== null) {
    ...
    /* called by parse() to initialize the rules & udts from the grammar object */ /* (the grammar object generated previously by apg) */ const initializeGrammar = function(grammar) {
        const functionName = "".concat(thisFileName, "initializeGrammar(): ");
        if (!grammar) {
            throw new Error("".concat(functionName, "grammar object undefined"));
        }
        if (grammar.grammarObject !== 'grammarObject') {
            throw new Error("".concat(functionName, "bad grammar object"));
        }
        rules = grammar.rules;
        udts = grammar.udts;
    };
    /* called by parse() to initialize the start rule */ const initializeStartRule = function(startRule) {
        const functionName = "".concat(thisFileName, "initializeStartRule(): ");
        let start = null;
        if (typeof startRule === 'number') {
            if (startRule >= rules.length) {
                throw new Error("".concat(functionName, "start rule index too large: max: ").concat(rules.length, ": index: ").concat(startRule));
            }
            start = startRule;
            ...
            throw new Error("".concat(functionName, "type of start rule '").concat(typeof startRule, "' not recognized"));
        }
        return start;
    };
    /* called by parse() to initialize the array of characters codes representing the input string */ const initializeInputChars = function initializeInputChars(inputArg, begArg, lenArg) {
        const functionName = "".concat(thisFileName, "initializeInputChars(): ");
        /* varify and normalize input */ let input = inputArg;
        let beg = begArg;
        let len = lenArg;
        if (input === undefined) {
            throw new Error("".concat(functionName, "input string is undefined"));
        }
        if (input === null) {
            throw new Error("".concat(functionName, "input string is null"));
        }
        if (typeof input === 'string') {
            input = utils.stringToChars(input);
        } else if (!Array.isArray(input)) {
            throw new Error("".concat(functionName, "input string is not a string or array"));
        }
        if (input.length > 0) {
            if (typeof input[0] !== 'number') {
                throw new Error("".concat(functionName, "input string not an array of integers"));
            }
        }
        /* verify and normalize beginning index */ if (typeof beg !== 'number') {
            ...
            if (beg < 0 || beg > input.length) {
                throw new Error("".concat(functionName, "input beginning index out of range: ").concat(beg));
            }
        }
        /* verify and normalize input length */ if (typeof len !== 'number') {
            ...
            if (len < 0 || len > input.length - beg) {
                throw new Error("".concat(functionName, "input length out of range: ").concat(len));
            }
        }
        chars = input;
        charsBegin = beg;
    ...
    /* called by parse() to initialize the user-written, syntax callback functions, if any */ const initializeCallbacks = function() {
        ...
        for(const index in thisThis.callbacks){
            ...
            if (typeof func === 'function' || func === null) {
                ...
                throw new Error("".concat(functionName, "syntax callback[").concat(index, "] must be function reference or 'false' (false/null/undefined/etc.)"));
            }
        }
        /* make sure all udts have been defined - the parser can't work without them */ for(i = 0; i < udts.length; i += 1){
            if (udtCallbacks[i] === null) {
                throw new Error("".concat(functionName, "all UDT callbacks must be defined. UDT callback[").concat(udts[i].lower, "] not a function reference"));
            }
        }
    };
    ...
    this.setMaxTreeDepth = function(depth) {
        if (typeof depth !== 'number') {
            throw new Error("parser: max tree depth must be integer > 0: ".concat(depth));
        }
    ...
    this.setMaxNodeHits = function(hits) {
        if (typeof hits !== 'number') {
            throw new Error("parser: max node hits must be integer > 0: ".concat(hits));
        }
    ...
    const validateUdtCallbackResult = function(udt, sysData, charsLeft) {
        ...
        switch(sysData.state){
            case id.ACTIVE:
                throw new Error("".concat(thisFileName, "opUDT(").concat(udt.name, "): callback function return error. ACTIVE state not allowed."));
            case id.EMPTY:
                if (udt.empty === false) {
                    throw new Error("".concat(thisFileName, "opUDT(").concat(udt.name, "): callback function return error. May not return EMPTY."));
                } else {
                    sysData.phraseLength = 0;
                }
                break;
    ...
    const opBKA = function(opIndex, phraseIndex, sysData) {
        ...
        switch(sysData.state){
            ...
            case id.NOMATCH:
                ...
                break;
            default:
                throw new Error("opBKA: invalid state ".concat(sysData.state));
        }
    };
    // The `BKN` operator.<br>
    ...
    const opBKN = function(opIndex, phraseIndex, sysData) {
        ...
        switch(sysData.state){
            ...
            case id.NOMATCH:
                ...
                break;
            default:
                throw new Error("opBKN: invalid state ".concat(sysData.state));
        }
    };
    ...
    opExecute = function opExecuteFunc(opIndex, phraseIndex, sysData) {
        ...
        if (nodeHits > limitNodeHits) {
            throw new Error("parser: maximum number of node hits exceeded: ".concat(limitNodeHits));
        }
        treeDepth += 1;
        if (treeDepth > maxTreeDepth) {
            maxTreeDepth = treeDepth;
            if (maxTreeDepth > limitTreeDepth) {
                throw new Error("parser: maximum parse tree depth exceeded: ".concat(limitTreeDepth));
            }
        }
        sysData.refresh();
        if (thisThis.trace !== null) {
...
"[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-lib/stats.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
...
module.exports = function statsFunc() {
    ...
    /* helper for toHtml() */ const displayRow = function displayRow(name, stat) {
        let html = '';
        html += '<tr>';
        html += '<td class="'.concat(style.CLASS_ACTIVE, '">').concat(name, "</td>");
        html += '<td class="'.concat(style.CLASS_EMPTY, '">').concat(stat.empty, "</td>");
        html += '<td class="'.concat(style.CLASS_MATCH, '">').concat(stat.match, "</td>");
        html += '<td class="'.concat(style.CLASS_NOMATCH, '">').concat(stat.nomatch, "</td>");
        html += '<td class="'.concat(style.CLASS_ACTIVE, '">').concat(stat.total, "</td>");
        html += '</tr>\n';
        return html;
    };
    ...
    /* helper for toHtml() */ const displayRules = function displayRules() {
        ...
        for(let i = 0; i < rules.length; i += 1){
            if (ruleStats[i].total > 0) {
                html += '<tr>';
                html += '<td class="'.concat(style.CLASS_ACTIVE, '">').concat(ruleStats[i].name, "</td>");
                html += '<td class="'.concat(style.CLASS_EMPTY, '">').concat(ruleStats[i].empty, "</td>");
                html += '<td class="'.concat(style.CLASS_MATCH, '">').concat(ruleStats[i].match, "</td>");
                html += '<td class="'.concat(style.CLASS_NOMATCH, '">').concat(ruleStats[i].nomatch, "</td>");
                html += '<td class="'.concat(style.CLASS_ACTIVE, '">').concat(ruleStats[i].total, "</td>");
                html += '</tr>\n';
            }
        }
        if (udts.length > 0) {
            ...
            for(let i = 0; i < udts.length; i += 1){
                if (udtStats[i].total > 0) {
                    html += '<tr>';
                    html += '<td class="'.concat(style.CLASS_ACTIVE, '">').concat(udtStats[i].name, "</td>");
                    html += '<td class="'.concat(style.CLASS_EMPTY, '">').concat(udtStats[i].empty, "</td>");
                    html += '<td class="'.concat(style.CLASS_MATCH, '">').concat(udtStats[i].match, "</td>");
                    html += '<td class="'.concat(style.CLASS_NOMATCH, '">').concat(udtStats[i].nomatch, "</td>");
                    html += '<td class="'.concat(style.CLASS_ACTIVE, '">').concat(udtStats[i].total, "</td>");
                    html += '</tr>\n';
                }
            }
        }
    ...
    this.toHtml = function toHtml(type, caption) {
        let html = '';
        html += '<table class="'.concat(style.CLASS_STATS, '">\n');
        if (typeof caption === 'string') {
            html += "<caption>".concat(caption, "</caption>\n");
        }
        html += '<tr><th class="'.concat(style.CLASS_ACTIVE, '">ops</th>\n');
        html += '<th class="'.concat(style.CLASS_EMPTY, '">EMPTY</th>\n');
        html += '<th class="'.concat(style.CLASS_MATCH, '">MATCH</th>\n');
        html += '<th class="'.concat(style.CLASS_NOMATCH, '">NOMATCH</th>\n');
        html += '<th class="'.concat(style.CLASS_ACTIVE, '">totals</th></tr>\n');
        const test = true;
        while(test){
            if (type === undefined) {
                html += displayOpsOnly();
...
"[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-lib/trace.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
...
module.exports = function exportTrace() {
    const utils = __turbopack_context__.r("[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-lib/utilities.js [app-client] (ecmascript)");
    const style = __turbopack_context__.r("[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-lib/style.js [app-client] (ecmascript)");
    const circular = new (__turbopack_context__.r("[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-lib/circular-buffer.js [app-client] (ecmascript)"))();
    const id = __turbopack_context__.r("[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-lib/identifiers.js [app-client] (ecmascript)");
    ...
    const ruleFilter = [];
    /* special trace table phrases */ const PHRASE_END = '<span class="'.concat(style.CLASS_LINEEND, '">&bull;</span>');
    const PHRASE_CONTINUE = '<span class="'.concat(style.CLASS_LINEEND, '">&hellip;</span>');
    const PHRASE_EMPTY = '<span class="'.concat(style.CLASS_EMPTY, '">&#120634;</span>');
    /* filter the non-RNM & non-UDT operators */ const initOperatorFilter = function() {
        ...
        for(const name in that.filter.operators){
            ...
            /* case 4: one or more individual operators specified: trace 'true' operators only */ if (upper === 'ALT') {
                ...
                throw new Error("".concat(thisFileName, "initOpratorFilter: '").concat(name, "' not a valid operator name.") + " Must be <all>, <none>, alt, cat, rep, tls, tbs, trg, and, not, bkr, bka or bkn");
            }
        }
    };
    /* filter the rule and `UDT` named operators */ const initRuleFilter = function() {
        ...
        for(const name in that.filter.rules){
            ...
            i = list.indexOf(lower);
            if (i < 0) {
                throw new Error("".concat(thisFileName, "initRuleFilter: '").concat(name, "' not a valid rule or udt name"));
            }
            ...
            if (opcode) {
                ...
                switch(opcode.type){
                    ...
                    case id.BKR:
                        ...
                        node.opData = "\\\\".concat(casetype).concat(modetype).concat(name);
                        break;
                    case id.TLS:
                        node.opData = [];
    ...
    /* From here on down, these are just helper functions for `toHtml()`. */ const htmlHeader = function(mode, caption) {
        ...
        switch(mode){
            ...
            case MODE_UNICODE:
                ...
                break;
            default:
                throw new Error("".concat(thisFileName, "htmlHeader: unrecognized mode: ").concat(mode));
        }
        let header = '';
        header += "<p>display mode: ".concat(modeName, "</p>\n");
        header += '<table class="'.concat(style.CLASS_TRACE, '">\n');
        if (typeof caption === 'string') {
            header += "<caption>".concat(caption, "</caption>");
        }
        return header;
    };
    const htmlFooter = function() {
        ...
        /* display a table legend */ footer += '<p class="'.concat(style.CLASS_MONOSPACE, '">legend:<br>\n');
        ...
        footer += '&nbsp;&nbsp;&nbsp;&nbsp;-&nbsp;<span class="'.concat(style.CLASS_ACTIVE, '">&darr;</span>&nbsp;&nbsp;phrase opened<br>\n');
        footer += '&nbsp;&nbsp;&nbsp;&nbsp;-&nbsp;<span class="'.concat(style.CLASS_MATCH, '">&uarr;M</span> phrase matched<br>\n');
        footer += '&nbsp;&nbsp;&nbsp;&nbsp;-&nbsp;<span class="'.concat(style.CLASS_EMPTY, '">&uarr;E</span> empty phrase matched<br>\n');
        footer += '&nbsp;&nbsp;&nbsp;&nbsp;-&nbsp;<span class="'.concat(style.CLASS_NOMATCH, '">&uarr;N</span> phrase not matched<br>\n');
        footer += 'operator&nbsp;-&nbsp;ALT, CAT, REP, RNM, TRG, TLS, TBS<sup>&dagger;</sup>, UDT, AND, NOT, BKA, BKN, BKR, ABG, AEN<sup>&Dagger;</sup><br>\n';
        footer += "phrase&nbsp;&nbsp;&nbsp;-&nbsp;up to ".concat(MAX_PHRASE, " characters of the phrase being matched<br>\n");
        footer += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-&nbsp;<span class="'.concat(style.CLASS_MATCH, '">matched characters</span><br>\n');
        footer += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-&nbsp;<span class="'.concat(style.CLASS_LOOKAHEAD, '">matched characters in look ahead mode</span><br>\n');
        footer += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-&nbsp;<span class="'.concat(style.CLASS_LOOKBEHIND, '">matched characters in look behind mode</span><br>\n');
        footer += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-&nbsp;<span class="'.concat(style.CLASS_REMAINDER, '">remainder characters(not yet examined by parser)</span><br>\n');
        footer += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-&nbsp;<span class="'.concat(style.CLASS_CTRLCHAR, '">control characters, TAB, LF, CR, etc. (ASCII mode only)</span><br>\n');
        footer += "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-&nbsp;".concat(PHRASE_EMPTY, " empty string<br>\n");
        footer += "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-&nbsp;".concat(PHRASE_END, " end of input string<br>\n");
        footer += "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-&nbsp;".concat(PHRASE_CONTINUE, " input string display truncated<br>\n");
        footer += '</p>\n';
        footer += '<p class="'.concat(style.CLASS_MONOSPACE, '">\n');
    ...
    /* format the TRG operator */ const displayTrg = function(mode, op) {
        ...
        if (op.type === id.TRG) {
            if (mode === MODE_HEX || mode === MODE_UNICODE) {
                ...
                html += "&ndash;".concat(hex);
            } else {
                html = "%d".concat(op.min.toString(10), "&ndash;").concat(op.max.toString(10));
            }
        }
        return html;
    };
    /* format the REP operator */ const displayRep = function(mode, op) {
        ...
        if (op.type === id.REP) {
            if (mode === MODE_HEX) {
                ...
                html = "".concat(op.min.toString(10), "&ndash;").concat(op.max.toString(10));
            } else {
                html = "".concat(op.min.toString(10), "&ndash;inf");
            }
        }
        return html;
    };
            ...
            if (mode === MODE_HEX || mode === MODE_DEC) {
                ...
                for(let i = 0; i < len; i += 1){
                    ...
                    if (charl >= 97 && charl <= 122) {
                        charu = charl - 32;
                        html += "".concat(charu.toString(base), "/").concat(charl.toString(base)).toUpperCase();
    ...
    const subPhrase = function(mode, charsArg, index, length, prev) {
        ...
        switch(mode){
            ...
            case MODE_DEC:
                if (prev) {
                    return ",".concat(utils.charsToDec(charsArg, index, length));
                }
    ...
    /* display phrases matched in look-behind mode */ const displayBehind = function(mode, charsArg, state, index, length, anchor) {
        ...
        const spanBehind = '<span class="'.concat(style.CLASS_LOOKBEHIND, '">');
        const spanRemainder = '<span class="'.concat(style.CLASS_REMAINDER, '">');
        const spanend = '</span>';
        let prev = false;
        switch(state){
            case id.EMPTY:
    ...
    /* display phrases matched in look-ahead mode */ const displayAhead = function(mode, charsArg, state, index, length) {
        const spanAhead = '<span class="'.concat(style.CLASS_LOOKAHEAD, '">');
    ...
    /* display phrases matched in normal parsing mode */ const displayNone = function(mode, charsArg, state, index, length) {
        const spanAhead = '<span class="'.concat(style.CLASS_MATCH, '">');
        return displayForward(mode, charsArg, state, index, length, spanAhead);
    };
    /* Returns the filtered records, formatted as an HTML table. */ const htmlTable = function(mode) {
        ...
        circular.forEach((lineIndex)=>{
            ...
            html += '<tr>';
            html += "<td>".concat(thisLine, "</td><td>").concat(thatLine, "</td>");
            html += "<td>".concat(line.phraseIndex, "</td>");
            html += "<td>".concat(line.phraseLength, "</td>");
            html += "<td>".concat(line.depth, "</td>");
            html += '<td>';
            switch(line.state){
                case id.ACTIVE:
                    html += '<span class="'.concat(style.CLASS_ACTIVE, '">&darr;&nbsp;</span>');
                    break;
                case id.MATCH:
                    html += '<span class="'.concat(style.CLASS_MATCH, '">&uarr;M</span>');
                    break;
                case id.NOMATCH:
                    html += '<span class="'.concat(style.CLASS_NOMATCH, '">&uarr;N</span>');
                    break;
                case id.EMPTY:
                    html += '<span class="'.concat(style.CLASS_EMPTY, '">&uarr;E</span>');
                    break;
                default:
                    html += '<span class="'.concat(style.CLASS_ACTIVE, '">--</span>');
                    break;
            }
            html += '</td>';
            html += '<td>';
            ...
            if (lookAhead) {
                html += '<span class="'.concat(style.CLASS_LOOKAHEAD, '">');
            } else if (lookBehind) {
                html += '<span class="'.concat(style.CLASS_LOOKBEHIND, '">');
            }
            html += utils.opcodeToString(line.opcode.type);
            if (line.opcode.type === id.RNM) {
                html += "(".concat(rules[line.opcode.index].name, ") ");
            }
            if (line.opcode.type === id.BKR) {
                ...
                html += "(\\".concat(casetype).concat(modetype).concat(rules[line.opcode.index].name, ") ");
            }
            if (line.opcode.type === id.UDT) {
                html += "(".concat(udts[line.opcode.index].name, ") ");
            }
            if (line.opcode.type === id.TRG) {
                html += "(".concat(displayTrg(mode, line.opcode), ") ");
            }
            if (line.opcode.type === id.TBS) {
                html += "(".concat(displayTbs(mode, line.opcode), ") ");
            }
            if (line.opcode.type === id.TLS) {
                html += "(".concat(displayTls(mode, line.opcode), ") ");
            }
            if (line.opcode.type === id.REP) {
                html += "(".concat(displayRep(mode, line.opcode), ") ");
            }
            if (lookAround) {
                html += '</span>';
            }
...
"[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-lib/node-exports.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
...
// ````
module.exports = {
    ast: __turbopack_context__.r("[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-lib/ast.js [app-client] (ecmascript)"),
    circular: __turbopack_context__.r("[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-lib/circular-buffer.js [app-client] (ecmascript)"),
    ids: __turbopack_context__.r("[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-lib/identifiers.js [app-client] (ecmascript)"),
    parser: __turbopack_context__.r("[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-lib/parser.js [app-client] (ecmascript)"),
    stats: __turbopack_context__.r("[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-lib/stats.js [app-client] (ecmascript)"),
    trace: __turbopack_context__.r("[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-lib/trace.js [app-client] (ecmascript)"),
    utils: __turbopack_context__.r("[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-lib/utilities.js [app-client] (ecmascript)"),
    emitcss: __turbopack_context__.r("[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-lib/emitcss.js [app-client] (ecmascript)"),
    style: __turbopack_context__.r("[project]/node_modules/.pnpm/apg-js@4.4.0/node_modules/apg-js/src/apg-lib/style.js [app-client] (ecmascript)")
};
}),
]);

//# sourceMappingURL=a251c_apg-js_src_4dada058._.js.map